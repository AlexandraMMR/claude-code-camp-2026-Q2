...
```bash
python scripts/mud_client.py connect --output response.txt
```
Opens Telnet session, auto-logs in, returns initial room description.
**Note**: This command connects and immediately disconnects. For gameplay, use `session`, `cmd`, or `run`.

### `cmd` - Send a single raw command
```bash
python scripts/mud_client.py cmd --command "look" --output response.txt
```
Connects, sends a single raw MUD command, returns response, then disconnects.
...
**User request**: "Connect to the MUD, check my character's HP and gold, then go north and look around."

**Agent workflow**:
1. `connect` → Get initial room description
2. `status` → Parse HP and gold from `score` output  
3. `run --commands "n" "look"` → Move north and examine new room
4. Write output to appropriate files
5. Update `data/player.md` and `data/world.md`
